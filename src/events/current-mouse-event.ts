...
export default class CurrentMouseEvent {
    ...
    constructor() {
        ...
        const captureListener = (queue) => (evt) => {
            queue.push(evt);
            /**
        ...
        this.getCurrentMouseEvent = () => {
            ...
            const md = getLatest(mousedownQueue);
            const mu = getLatest(mouseupQueue);
            const cl = getLatest(clickQueue);
            const evt = [cl, md, mu].sort(compareTimestamp)[0];
